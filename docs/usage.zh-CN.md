...
无论是否保留旧脚本标记，实际键名和表头中的多余引号都会进行一次兼容性规范化并备份，
之后再次运行保持字节不变。普通表头的尾部注释移到前一行保留，避免编辑器漏认表边界。
...
等常规写法，只有 TOML 语法要求时才给键名加引号。处理不依赖旧脚本注释，
覆盖全带引号、部分带引号、单引号、表头内部空格和尾部注释等写法。

规范化前后都会解析并校验配置含义一致；扫描会跳过字符串内容、数组和内联值，
不会把提示词、URL、Key 或注释中类似表头的文字当作真实配置修改。
仅需键名规范化时保留其余内容和注释；内联 provider、点号赋值、数组表或编辑器无法识别的
多行关键字段，会一次性展开为常规格式并保存原始备份，配置值保持不变。
无需再规范化时保持文件原始字节。

CC Switch 的文本编辑仍要求供应商 ID 可用常规表路径表达，例如 `custom`。
包含点号或空格、必须使用引号的特殊 ID 会保留其 TOML 含义，不会擅自改名；
这类配置不在本次保存函数兼容验证范围内，不能据此保证编辑器支持。
